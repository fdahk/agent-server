...
栈：NestJS(`@nestjs/*`, swagger, passport-jwt) + Prisma + ioredis + BullMQ + Qdrant + OpenAI；校验 class-validator/transformer；JWKS 验签 `jwks-rsa`。
...
- 异步任务走 BullMQ worker；向量检索走 Qdrant；LLM 走 OpenAI（注意限流/超时/降级）
