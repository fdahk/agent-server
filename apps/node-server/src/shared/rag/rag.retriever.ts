 ...
 *
 * 多租户隔离的纪律:检索只能走这里,且只经 milvus.searchByUser(user_id 过滤写死
 * 在那一层)。禁止任何 controller/service 自己拼 Milvus filter——从代码结构上杜绝
 * 漏过滤导致的越权泄露。
 */
@Injectable()
export class RagRetriever {
  ...
  async retrieve(
    ...
    const hits = await this.milvus.searchByUser(vector, userId, topK);
