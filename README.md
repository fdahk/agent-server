...
只需要安装 [Docker](https://www.docker.com/) 和 [Ollama](https://ollama.com/)。

```bash
# 1. 确保 Ollama 在宿主机运行并已拉取模型
ollama pull qwen2.5:7b

# 2. 创建环境变量文件
cp .env.docker.example .env.docker

# 3. 一键启动全部服务（MySQL + Redis + RabbitMQ + Node + Java）
docker compose --env-file .env.docker up -d

# 4. 查看启动日志（等待所有服务 healthy）
docker compose --env-file .env.docker logs -f

# 5. 验证
curl http://localhost:3000/api                    # Java Core 健康检查
curl http://localhost:3101/api                    # Node AI Gateway 健康检查
curl http://localhost:3000/actuator/health        # Actuator 详细健康状态
```
...
| 服务 | 地址 | 说明 |
|------|------|------|
| Java Core API | http://localhost:3000/api | 对外主入口 |
| Swagger 文档 | http://localhost:3000/swagger-ui.html | API 交互式文档 |
| Node AI Gateway | http://localhost:3101/api | 内部 AI 网关 |
| RabbitMQ 管理面板 | http://localhost:15672 | 默认账号 guest/guest |

**常用 Docker Compose 命令：**

```bash
docker compose --env-file .env.docker up -d          # 后台启动
docker compose --env-file .env.docker logs -f         # 查看实时日志
docker compose --env-file .env.docker logs java-server # 只看 Java 日志
docker compose --env-file .env.docker ps              # 查看服务状态
docker compose --env-file .env.docker restart java-server  # 重启单个服务
docker compose --env-file .env.docker down            # 停止并移除容器
docker compose --env-file .env.docker down -v         # 停止并清除数据卷
```
...
- [x] **Docker Compose 编排**：一键启动 Java + Node + MySQL + Redis + RabbitMQ
